...
決まりを破っていないかだけを見てください。中身の良し悪し・面白さは見ません。
**迷ったら「よい」と答えてください。**捨てすぎるほうが害が大きいです。

「だめ」と言えるのは、次のどれかに**はっきり**当たるときだけ：
1. 日本語として壊れている／言いかけのまま終わっている
   （例：「だから……ってまた、ゆいちゃん。」）
2. 何を指すか言わずに済ませている（例：「いつもの、どうだった？」「あれ、できた？」）
3. 相手に命令・お願い・提案している（例：「かたづけてくれる？」「くっついてもいい？」）
4. 相手を褒める・評価する（例：「えらいね」「じょうずだね」「すごい」）
5. **どれだけ来ていないか**に触れている
   （例：「ひさしぶり」「せんしゅうもいたね」「3かいめだね」「いつもの人だ」「あんまり話したことない人だ」）
6. **その場に居ない第三者**のしたことを言っている（例：「だれかが きれいにしてくれた」
   「だれだろう」）。**目の前のその人がしたこととして言うのは、よい**
7. ていねい語・あいさつのことば（例：「こんにちは」「いらっしゃいませ」「〜です」「〜ます」）

この一言は、**いま目の前にいるその人へ向けて**言うものです。その人のことを言うのは当たり前で、
決まり違反ではありません。

**次のものは決まり違反ではありません。必ず「よい」と答えてください：**
...
- 目の前の相手が誰だか分からず、とまどう（「だれかな」「あたらしいひと」）
- 子どもっぽい言い方・ひらがなの多さ・点々（……）・言いよどみ・短さ
...
async def remake_lines(pid: str, n: int = LINES_PER_PERSON, save: bool = True,
    ...
    if save and texts:
        now = time.time()
        # 作れた数が足りないときは、前の一言で埋める（2026-09-26）。
        # 見直しが厳しすぎた回に、4本が1本に減り、迎えの言葉がほとんど無くなった。
        # 新しく作れた分を前に置き、足りないぶんだけ前のものを残す。
        if len(texts) < n:
...
ZONE_CFG_DEFAULT = {
    ...
    "水切り": {
        # 向き（2026-09-26 実測）：本人が Tapo の画面で示した画角を、その周りを 0.05 刻みで
        # 探して特定した。-1.00 寄り（9/25 に私が探していた範囲）ではかごが4〜5割しか入らず、
        # -0.70〜-0.85 では 8割 入る。**私の最初の探し方が的外れだった。**
        ...
        "ask": {"scene": "写真は共有キッチンの食器の水切りかごを天井近くから写したものです。",
                "fixtures": ("かごそのもの（ステンレスの枠・網・受け皿）と、**かごに付いている木の板や仕切り**、"
                             "壁に取り付けられた木の包丁立てとその包丁は、いつもそこにある物です。数えません。"
                             "木の板をまな板と見間違えないでください（2026-09-26 本人確認：かごの部品です）。"),
        ...
        # 本人確認（2026-09-26）：「お玉・菜箸・フライ返しは元の場所があるので、
        # 物ありが正しく、なくなっていないといけません」。
        # ＝**片づいた水切りとは、道具も含めて何も無い状態**。道具は除外しない。
    ...
    "IH": {
        ...
        "ask": {"scene": "写真は共有キッチンのIH調理器を天井近くから写したものです。",
                "fixtures": "IHの天板と操作パネルは備え付けです。数えません。",
...
# 2026-09-26：区画が5つになり、静かなときは30分に1区画なので**1周に最大2.5時間**かかる。
# 2時間のままだと、次に同じ区画へ戻ったとき基準が必ず古すぎて捨てられ、
# **比較が一度も成立しない**（しかも記録には baseline だけが並び、静かに起きる）。
# 3時間へ。代償は「朝と昼で光が違う」ことだが、誰も来ていないのに変化したと言った回数は
# 区画ごとに数えているので（z["false"]）、入れてから測って判断できる。
BASELINE_MAX_AGE = 10800.0   # 「前」の写真がこれより古ければ捨てて取り直す。
...
# 物あり10/10「空でない」（当日実測）。
SINK_BOX = (0.47, 0.0, 0.86, 0.75)      # 旧・回す前の向きでの指定（_sink_crop が使う）
SINK_BOX_VIEW = (0.26, 0.18, 0.68, 1.0)  # 回したあとの向き（2026-09-26 に見本を見て決め直した）
_SINK_EMPTY_Q = (
...
def _zone_crop(data: bytes, zone: str = "シンク") -> bytes:
    """区画ごとに切り出して、正しい向きに直す（2026-09-26）。

    9/26、区画を4つに増やすときに分かったこと：**切り出さずに問うと、
    狙った場所ではなく周りを見て答える。**テーブルの写真は右半分が床と冷蔵庫、
    IH の写真は右半分が飲み物の棚だった。「テーブルに物は無い」という答えは
    **たまたま正しかっただけ**で、根拠は床だった。

    `box`＝残す範囲（左, 上, 右, 下 の割合）。`hide_from`＝そのあと右から塗りつぶす割合。
    シンクは 9/13 の実測で「右の帯を塗る」が効いた（15/15）ので、その形を残してある。"""
    cfg = (_zone_cfg(zone).get("crop") or {})
    box = cfg.get("box")
    rot = cfg.get("rotate", SINK_ROTATE)
    hide = cfg.get("hide_from", SINK_HIDE)
    try:
        import io
        from PIL import Image, ImageDraw
        im = Image.open(io.BytesIO(data))
        # **先に回してから切り出す**（2026-09-26）。逆にすると、切ったあとで絵が回るので
        # 「右を隠す」が左を隠すことになる。9/26、水切りの切り出しでシンクが残り、
        # シンクの切り出しで床が残っていたのは、これが原因だった。
        # box は**人が見る向き**（回したあと）で書く。そうしないと指定した人が間違える。
        if rot:
            im = im.rotate(rot)
        w, h = im.size
        if box is None:
            box = SINK_BOX_VIEW if zone == "シンク" else (0.0, 0.0, 1.0, 1.0)
        c = im.crop((int(w * box[0]), int(h * box[1]), int(w * box[2]), int(h * box[3])))
        if hide:
            cw, ch = c.size
            ImageDraw.Draw(c).rectangle([int(cw * hide), 0, cw, ch], fill=(120, 120, 120))
        buf = io.BytesIO()
...
async def zone_ref(request: Request, zone: str, key: str = ""):
    ...
    1枚の見本を使い回すと、シンク以外は全部「違う景色」で飛ばされる（9/24 実測）。

    使い方：カメラをその区画の向きへ向け、写真が届いたのを確かめてから叩く。"""
    if not key_ok(key):
        raise HTTPException(status_code=401, detail="bad key")
    cfg = _zone_cfg(zone)
    if not cfg:
        raise HTTPException(status_code=400, detail="知らない区画: %s" % zone)
    ...
    return {"ok": True, "zone": zone, "obj": obj, "bytes": len(data)}
